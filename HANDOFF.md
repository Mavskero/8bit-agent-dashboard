...
- Codex 来源会让左下角模块显示 `AGENT`，读取当前 rollout 的 `Reasoning`、`CommandExecution`、`FileChange`、`Extension`、MCP 和 `AgentMessage` 条目。界面使用九种彩色动作标签、合并相邻同类事件，并逐字显示精简正文；不渲染长命令参数和原始工具输出。
